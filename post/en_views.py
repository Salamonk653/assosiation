...
class Navbar(ContextMixin):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['navbar'] = merop.category_set.filter(language__name__icontains=u'en')
        context['anonsy'] = Category.objects.get(name__icontains=u'Анонсы')
        context['articles_4'] = New.objects.filter(language__name__icontains='en',
                                                   category__name__icontains=u'Жаңылыктар', is_public=True).order_by(
            '-id')[:5]
        context['form'] = ContactForm()
        context['assosiations'] = OFonde.objects.filter(language__name__icontains='en').first()
        return context


class News(ListView, Navbar):
    model = New
    template_name = u'en/index.html'

    def get_context_data(self, *args, **kwargs):
        context = super(News, self).get_context_data(**kwargs)
        context['news'] = New.objects.filter(category__name__iexact=u'Жаңылыктар', language__name__icontains=u'en',
                                             is_public=True).order_by('-id')[:3]
        context['merop'] = New.objects.filter(category__name__iexact=u'Анонсы', language__name__icontains=u'en',
                                              is_public=True).order_by('-id')[:3]
        context['projects'] = New.objects.filter(category__glavcategory__name__icontains=u'Наши проекты',
                                                 language__name__icontains=u'en', is_public=True).order_by('-id')[:8]
        index = []
        for i in range(1, context['projects'].count() + 1):
            index.append(i)
        context['index'] = index
        context['sliders'] = GlavSlider.objects.filter(language__name__icontains=u'en')
...
class Podrobnee(TemplateView, Navbar):
    template_name = 'en/podrobnee.html'

    def get_context_data(self, *args, **kwargs):
        context = super(Podrobnee, self).get_context_data(**kwargs)
        context['h2'] = 'What gives membership in IAPEF?'
        return context


class NewsList(ListView, Navbar):
    model = New
    template_name = u'en/novosti.html'
    queryset = New.objects.filter(category__name__icontains=u'Жаңылыктар', language__name__icontains='en').order_by(
        '-id')
    paginate_by = 6
...
class AnonsyList(ListView, Navbar):
    model = New
    template_name = u'en/anonsy.html'
    queryset = New.objects.filter(category__name__icontains=u'Анонсы', language__name__icontains='en').order_by('-id')
    paginate_by = 6
    context_object_name = u'category'

    def get_context_data(self, *args, **kwargs):
        context = super(AnonsyList, self).get_context_data(**kwargs)
        context['h2'] = 'Announcements'
        return context


class AnonsyDetail(DetailView, Navbar):
    model = New
    template_name = 'en/anonsy_detail.html'
    category = Category

    def get_context_data(self, *args, **kwargs):
        context = super(AnonsyDetail, self).get_context_data(**kwargs)
        context['articles'] = self.model.objects.filter(language__name__icontains='en',
                                                        category__name__icontains=u'Жаңылыктар',
                                                        is_public=True).order_by('-id')[:15]
        context['projects'] = New.objects.filter(category__glavcategory__name__icontains=u'Наши проекты',
                                                 language__name__icontains=u'en', is_public=True).order_by('-id')[:15]
...
class Projects(ListView, Navbar):
    ...
    queryset = New.objects.filter(category__glavcategory__name__icontains=u'Наши проекты',
                                  language__name__icontains=u'en', is_public=True).order_by('-id')
    context_object_name = u'projects'
    paginate_by = 6
    template_name = 'en/proejects.html'

    def get_context_data(self, *args, **kwargs):
        context = super(Projects, self).get_context_data(**kwargs)
        context['h2'] = u'Our projects'
        return context


class ProjectList(DetailView, Navbar):
    model = Category
    template_name = 'en/nashi_proecty.html'

    def get_context_data(self, *args, **kwargs):
        context = super(ProjectList, self).get_context_data(**kwargs)
        category = self.get_object()
        context['category'] = category.new_set.filter(language__name__icontains='en').order_by('-id')
        context['h2'] = self.get_object()
        return context
...
class ArticleDetail(DetailView, Navbar):
    model = New
    template_name = 'en/novosti_detail.html'
    category = Category
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['projects'] = New.objects.filter(category__glavcategory__name__icontains=u'Наши проекты',
                                                 language__name__icontains=u'en', is_public=True).order_by('-id')[:8]
        context['article'] = self.get_object()
        context['ofonde'] = OFonde.objects.filter(language__name__icontains='en').first()
        context['h2'] = self.get_object()
        return context


class Chlenstvo(TemplateView, Navbar):
    template_name = 'en/chlenstvo.html'

    def get_context_data(self, *args, **kwargs):
        context = super(Chlenstvo, self).get_context_data(**kwargs)
        context['h2'] = 'Membership'
        return context
...
class Contacts(TemplateView, Navbar):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['h2'] = 'Our contacts'
        return context


class Onas(TemplateView, Navbar):
    template_name = 'en/onas.html'

    def get_context_data(self, *args, **kwargs):
        context = super(Onas, self).get_context_data(**kwargs)
        context['h2'] = 'ABOUT US'
        context['fond'] = Fondjonundo.objects.filter(language__name__icontains='en').order_by('id')
        return context
...
class Search(View):
    ...
    merop = GlavCategory.objects.get(name__icontains=u'Наши проекты')
    context = {'assosiations': OFonde.objects.filter(language__name__icontains='en').first(),
               'navbar': merop.category_set.filter(language__name__icontains=u'en'),
               'anonsy': Category.objects.get(name__icontains=u'Анонсы'),
               'articles_4': New.objects.filter(language__name__icontains='en', category__name__icontains=u'Жаңылыктар',
    ...
    def get(self, request):
        ...
        q = request.GET.get('q')
        if q:
            self.context['posts'] = New.objects.filter(name__icontains=q, language__name__icontains='en')
        else:
            self.context['posts'] = ''
        # return context
...
def contact_form(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sent = True
            try:
                send_mail(subject, message, 'sulaiman.97_kg@mail.en', mailto)
...
class Katalog(TemplateView, Navbar):
    template_name = 'en/katolog.html'

    def get_context_data(self, *args, **kwargs):
        context = super(Katalog, self).get_context_data(**kwargs)
        context['h2'] = 'CATALOG'
        glavnyi = GlavCategoryForKatalog.objects.filter(language__name__icontains=u'en')
        context['glavnyi'] = glavnyi
        return context


class KatalogDetail(Search):
    template_name = 'en/katolog_detail.html'

    def get(self, request, slug):
        glavnyi = PostForKatalog.objects.filter(category__glavcategory__slug__icontains=slug,
                                                language__name__icontains=u'en') or \
                  PostForKatalog.objects.filter(category__slug__icontains=slug,
